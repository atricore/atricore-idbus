...
import org.atricore.idbus.capabilities.management.main.transform.IdApplianceTransformationContext;
...
import org.atricore.idbus.capabilities.management.support.springmetadata.model.osgi.Reference;
import org.atricore.idbus.capabilities.management.support.springmetadata.model.osgi.Service;
...
public class IdauBaseComponentsTransformer extends AbstractTransformer {
    ...
    public void before(TransformEvent event) {
        ...
        IdentityApplianceDefinition idbus = (IdentityApplianceDefinition) event.getData();
        IdApplianceTransformationContext context = event.getContext();
        IdProjectModule module = context.getCurrentModule();

        Date now = new Date();
        Beans idauBeans = newBeans(idbus.getName() + " : Configuration generated by Atricore Identity Bus Server on " + now.toGMTString());
        Beans idauBeansOsgi = newBeans(idbus.getName() + " : OSGi Configuration generated by Atricore Identity Bus Server on " + now.toGMTString());

        event.getContext().put("beans", idauBeans);
        event.getContext().put("beansOsgi", idauBeansOsgi);

        if (logger.isDebugEnabled())
            logger.debug("Defining Identity Appliance Unit base components based on " + idbus.getName());
