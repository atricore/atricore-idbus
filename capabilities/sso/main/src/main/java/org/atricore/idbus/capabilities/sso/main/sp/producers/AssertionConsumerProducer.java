...
import org.atricore.idbus.capabilities.sso.main.common.*;
 ...
 * @version $Id$
 */
public class AssertionConsumerProducer extends AbstractAssertionConsumerProducer {
    ...
    @Override
    protected Response processResponse(CamelMediationExchange exchange, Request originalRequest) throws Exception {
        // Incomming message
        ...
        if (status.equals(StatusCode.TOP_RESPONDER) &&
            ...
            return new ResponseImpl<SPAuthnResponseType>(ssoResponse.getID(), ssoResponse, true);
        ...
        return new ResponseImpl<ResponseType>(response.getID(), response, false);
    }

    @Override
    protected Subject extractIdPSubject(Response response) {
        return buildSubjectFromSamlR2Response((ResponseType) response.getMessage());
    }

    private Subject buildSubjectFromSamlR2Response(ResponseType response) {
