...
public class CscaMediator extends AbstractCamelMediator {

    private static final Log logger = LogFactory.getLog(CscaMediator.class);

    private String principal;
    private String realm;

    public CscaMediator() {
        logger.info("CscaMediator Instantiated");
    }

    @Override
    protected RouteBuilder createClaimRoutes(final ClaimChannel claimChannel) throws Exception {
        logger.info("Creating CSCA Routes");
        ...
        return new RouteBuilder() {
            ...
            public void configure() throws Exception {
                ...
                for (IdentityMediationEndpoint endpoint : endpoints) {
                    ...
                    CscaBinding binding = CscaBinding.asEnum(endpoint.getBinding());
                    ...
                    switch (binding) {
                        case SSO_ARTIFACT:
                            ...
                            // FROM idbus-bind TO csca (claim processing)
                            ...
                            if (ed.getResponseLocation() != null) {
                                ...
                                // FROM idbus-bind TO csca (token negotiation)
                            ...
                            break;
                        default:
                            throw new CscaException("Unsupported Csca Binding " + binding.getValue());
                    }
    ...
    public EndpointDescriptor resolveEndpoint(Channel channel, IdentityMediationEndpoint endpoint) throws IdentityMediationException {
        ...
        if (channel instanceof ClaimChannel) {
            ...
            CscaBinding binding = null;

            logger.debug("Creating CSCA Endpoint Descriptor for : " + endpoint.getName());

            // ---------------------------------------------
            // Resolve Endpoint binding
            // ---------------------------------------------
            if (endpoint.getBinding() != null)
                binding = CscaBinding.asEnum(endpoint.getBinding());
            else
                logger.warn("No CSCA Binding found in endpoint " + endpoint.getName());
