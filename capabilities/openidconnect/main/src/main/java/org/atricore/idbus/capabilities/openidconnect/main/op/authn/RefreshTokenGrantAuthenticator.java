...
public class RefreshTokenGrantAuthenticator extends AbstractSecurityTokenAuthenticator {

    private static final Log logger = LogFactory.getLog(RefreshTokenGrantAuthenticator.class);

    public static final String SCHEME_NAME = "oidc-refreshtokengrant-authentication";

    public RefreshTokenGrantAuthenticator() {
        super(SCHEME_NAME);
    ...
    protected Credential[] getCredentials(Object requestToken) throws SSOAuthenticationException {
        ...
        try {
            ...
            Credential c = new RefreshTokenGrantCredential(authzGrant);
    ...
    public boolean canAuthenticate(Object requestToken) {
        if (requestToken instanceof BinarySecurityTokenType) {
            BinarySecurityTokenType authzGrantToken = (BinarySecurityTokenType) requestToken;
            return authzGrantToken.getOtherAttributes().containsKey( new QName( REFRESH_TOKEN_NS) );
        }
