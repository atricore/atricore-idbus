...
package org.atricore.idbus.kernel.authz.util
...
/**
 * Helper methods to create a  [[org.atricore.idbus.kernel.authz.util.Resource.support.Resource]] from various sources
 */
object Resource {

  /**
   * Creates a [[org.atricore.idbus.kernel.authz.util.Resource.support.Resource]] from the actual String contents using the given
   * URI.
   *
   * The URI is used to determine the package name to put the policy in along with
   * the policy kind (using the extension of the URI)
   */
  def fromText(uri: String, policyText: String) = StringResource(uri, policyText)

  /**
   * Creates a [[org.atricore.idbus.kernel.authz.util.Resource.support.Resource]] from a local URI such as in a web application using the
  ...
  /**
   * Creates a [[org.atricore.idbus.kernel.authz.util.Resource.support.Resource]] from a file
   */
  ...
  /**
   * Creates a [[org.atricore.idbus.kernel.authz.util.Resource.support.Resource]] from a file name
   */
  def fromFile(fileName: String): FileResource = fromFile(new File(fileName))

  /**
   * Creates a [[org.atricore.idbus.kernel.authz.util.Resource.support.Resource]] from a URL
   */
  ...
  /**
   * Creates a [[org.atricore.idbus.kernel.authz.util.Resource.support.Resource]] from the [[scala.io.Source]] and the given URI.
   *
   * The URI is used to determine the package name to put the policy in along with
   * the policy kind (using the extension of the URI)
   */
