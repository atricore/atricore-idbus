Feature: Enable user and entitlement record lookup from an LDAP directory
  ...
  I want to associate an identity provider with an ldap identity source element

  Scenario: Associate an identity provider with an ldap identity source element
    Given I am on the modeler screen
    And an identity provider 'Sample IdP' exists in my diagram
    And an identity source 'Sample Ldap' exists in my diagram
    ...
    And I set the 'Sample Ldap' ldap identity source element as the target
